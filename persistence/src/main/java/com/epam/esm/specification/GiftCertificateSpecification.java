...
public class GiftCertificateSpecification {
    ...
    public Specification<GiftCertificate> certificateNameLike(String name) {
        return (root, query, cb) -> cb.like(
                cb.upper(root.get(NAME)), PERCENT_SIGN + name.toUpperCase() + PERCENT_SIGN
        );
    }

    public Specification<GiftCertificate> certificateDescriptionLike(String description) {
        return (root, query, cb) -> cb.like(
                cb.upper(root.get(DESCRIPTION)), PERCENT_SIGN + description.toUpperCase() + PERCENT_SIGN
        );
    }

    public Specification<GiftCertificate> certificateTagsIn(Set<String> tags) {
